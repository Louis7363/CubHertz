...
class AudioToVideo() :
    ...
    def generer_images_amplitude_carre(self) :
            for i in range (0,len(self.signal_audio_amplitude),self.pas ) :
                cote, im, draw = formes.carre(self.signal_audio_amplitude[i],self.max)
                draw.rectangle(xy=(250-int(cote/2),150-int(cote/2),(250+int(cote/2),150+int(cote/2))), fill=(255, 0, 0), width=cote)
